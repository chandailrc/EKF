...
% >>>>>>>>START: EKF PARAMETERS<<<<<<<<<<<<<
    ...
    gt_meas = [gt(1,:)+randn(NN,1)'; gt(2,:)+randn(NN,1)'; gt(3,:)+0.1*randn(NN,1)'];
    n_states=6;
    q_proc=0.1;                                 %std of process 
    r_meas=0.1;                                 %std of measurement
    n_meas_states = 6;                          % x, y, theta
    ...
    y_handle=@(x)[x(1);x(3);x(5);x(1);x(3);x(5)];
...
for frame=first_frame:last_frame
  ...
  if k>1
    plot([Tr_total{k-1}(1,4) Tr_total{k}(1,4)], ...
      ...
      meas = [Tr_total{k}(1,4); Tr_total{k}(3,4);eulAng(3,k); gt_meas(1,k);gt_meas(2,k);gt_meas(3,k)];
